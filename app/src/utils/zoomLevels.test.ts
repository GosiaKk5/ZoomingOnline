...
describe("zoomLevels utilities", () => {
  describe("generateZoomLevels", () => {
    describe("input validation", () => {
      it("should throw error for non-positive lowerLimit", () => {
        expect(() => generateZoomLevels(0, 0.001)).toThrow(
          "Both limits must be positive numbers",
        );
        expect(() => generateZoomLevels(-1e-6, 0.001)).toThrow(
          "Both limits must be positive numbers",
        );
      });

      it("should throw error for non-positive upperLimit", () => {
        expect(() => generateZoomLevels(1e-6, 0)).toThrow(
          "Both limits must be positive numbers",
        );
        expect(() => generateZoomLevels(1e-6, -0.001)).toThrow(
          "Both limits must be positive numbers",
      ...
      it("should throw error when lowerLimit >= upperLimit", () => {
        expect(() => generateZoomLevels(0.001, 0.001)).toThrow(
          "lowerLimit must be smaller than upperLimit",
        );
        expect(() => generateZoomLevels(0.002, 0.001)).toThrow(
          "lowerLimit must be smaller than upperLimit",
        );
      });
    });
    ...
    describe("zoom level generation for example case: 100ps resolution, 1ms segment", () => {
      ...
      it("should generate correct zoom levels", () => {
        ...
        expect(zoomLevels[zoomLevels.length - 1]!).toBeLessThanOrEqual(
          upperLimit / 2,
        );

        // Should include some key expected levels that we know should be there
        expect(
          zoomLevels.some((level) => Math.abs(level - 200e-12) < 1e-15),
        ).toBe(true); // 200ps
        expect(zoomLevels.some((level) => Math.abs(level - 1e-9) < 1e-15)).toBe(
          true,
        ); // 1ns
        expect(zoomLevels.some((level) => Math.abs(level - 1e-6) < 1e-15)).toBe(
          true,
        ); // 1us
      });
  ...
  describe("generateZoomLevelsWithLabels", () => {
    it("should generate zoom levels with human-readable labels", () => {
      const lowerLimit = 100e-12; // 100ps in seconds
      const upperLimit = 1e-6; // 1us in seconds
      const zoomLevelsWithLabels = generateZoomLevelsWithLabels(
        lowerLimit,
        upperLimit,
      );
    ...
    it("should have matching values between regular and labeled versions", () => {
      ...
      const labeledZoomLevels = generateZoomLevelsWithLabels(
        lowerLimit,
        upperLimit,
      );
